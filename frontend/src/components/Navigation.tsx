...
export default function Navigation({ activeTab, setActiveTab, isConnected }: NavigationProps) {
    ...
    <header className="flex justify-between items-center bg-neutral-950 border-b border-neutral-800 p-4 sticky top-0 z-50">
      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-2 bg-neutral-900 border border-neutral-800 px-3 py-1.5 rounded-full">
          <div className={`h-2.5 w-2.5 rounded-full ${isConnected ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)] animate-pulse" : "bg-red-500"}`}></div>
          <span className="text-xs font-mono text-neutral-400">SYS <span className={isConnected ? "text-green-400 font-bold" : "text-red-400 font-bold"}>{isConnected ? "NOMINAL" : "OFFLINE"}</span></span>
        </div>
